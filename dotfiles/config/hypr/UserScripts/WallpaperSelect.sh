...
modify_startup_config() {
  ...
  if [[ "$selected_file" =~ \.(mp4|mkv|mov|webm)$ ]]; then
    ...
    sed -i --follow-symlinks '/^\s*exec-once\s*=\s*awww-daemon\s*--format\s*xrgb\s*$/s/^/\#/' "$startup_config"
    ...
    sed -i --follow-symlinks '/^\s*#\s*exec-once\s*=\s*awww-daemon\s*--format\s*xrgb\s*$/s/^\s*#\s*//;' "$startup_config"
